defmodule Aoc2019.Day02 do
  ...
  def part_one do
    ...
    input
    |> Enum.chunk_every(4)
    |> Enum.reduce_while(input, fn cur, acc -> process(cur, acc) end)
  end
  ...
  defp process([1, a, b, c], list) do
    result = Enum.at(list, a) + Enum.at(list, b)

    {:cont, List.replace_at(list, c, result)}
  end

  defp process([2, a, b, c], list) do
    result = Enum.at(list, a) * Enum.at(list, b)

    {:cont, List.replace_at(list, c, result)}
  end

  defp process([99 | _], list), do: {:halt, list}

  defp load_input do
    :code.priv_dir(:aoc2019)
    |> Path.join("day_02.txt")
    |> File.read!()
  end
